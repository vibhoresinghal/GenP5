...
function draw() {
    ...
    mandala: {
        ...
        controls: [
            { group: 'Geometry', icon: 'layers' },
            { name: 'layers', label: 'Complexity Layers', type: 'slider', min: 2, max: 24, step: 1 },
            { name: 'segments', label: 'Symmetry Points', type: 'slider', min: 4, max: 48, step: 1 },
            { name: 'innerRadius', label: 'Inner Core', type: 'slider', min: 0, max: 0.8, step: 0.05 },
            { name: 'scale', label: 'Overall Scale', type: 'slider', min: 0.2, max: 2.5, step: 0.1 },
            { group: 'Effect', icon: 'sparkles' },
            { name: 'distortion', label: 'Noise Distortion', type: 'slider', min: 0, max: 50, step: 1 },
            { name: 'shapeVariety', label: 'Shape Variation', type: 'slider', min: 0, max: 3, step: 1 },
            { name: 'showGlow', label: 'Bloom Effect', type: 'toggle' },
            { group: 'Motion', icon: 'play' },
            { name: 'rotationSpeed', label: 'Rotation Speed', type: 'slider', min: 0, max: 3, step: 0.1 },
            { name: 'animate', label: 'Enable Rotation', type: 'toggle' },
            { group: 'Styling', icon: 'palette' },
            { name: 'strokeWeight', label: 'Line Border', type: 'slider', min: 0.5, max: 6, step: 0.5 },
            {
                name: 'colorMode', label: 'Theme Palette', type: 'palette', options: [
        ...
        draw: function (p, params) {
            ...
            const startRadius = maxRadius * params.innerRadius;

            for (let layer = 0; layer < params.layers; layer++) {
                const layerT = layer / params.layers;
                const radius = p.lerp(startRadius, maxRadius, layerT);
                const layerRotation = this.rotation * (layer % 2 === 0 ? 1 : -1) * (1 + layer * 0.2);
                ...
                if (params.showGlow) {
                    ...
                    p.drawingContext.shadowColor = p.color(p.red(c), p.green(c), p.blue(c), 100).toString();
                } else {
                ...
                for (let i = 0; i < params.segments; i++) {
                    ...
                    let d = params.distortion > 0 ? p.noise(layer, i, p.frameCount * 0.01) * params.distortion : 0;
                    let r = radius + d;

                    const shapeType = Math.floor(params.shapeVariety + layer) % 4;

                    if (shapeType === 0) {
                        p.ellipse(r * 0.8, 0, r * 0.2, r * 0.2);
                    } else if (shapeType === 1) {
                        p.line(0, 0, r, 0);
                        p.rect(r - 5, -5, 10, 10);
                    } else if (shapeType === 2) {
                        ...
                        p.arc(0, 0, r * 2, r * 2, 0, p.PI / params.segments);
                    }
...
function draw() {
    ...
    recursiveTree: {
        ...
        controls: [
            { group: 'Growth', icon: 'tree' },
            { name: 'depth', label: 'Recursion Depth', type: 'slider', min: 4, max: 13, step: 1 },
            { name: 'initialLength', label: 'Trunk Height', type: 'slider', min: 60, max: 250, step: 10 },
            { name: 'branchAngle', label: 'Spread Angle', type: 'slider', min: 5, max: 60, step: 1 },
            { name: 'branchRatio', label: 'Shrink Factor', type: 'slider', min: 0.4, max: 0.85, step: 0.01 },
            { name: 'randomness', label: 'Chaos/Variety', type: 'slider', min: 0, max: 20, step: 1 },
            { group: 'Secondary', icon: 'sparkles' },
            { name: 'showLeaves', label: 'Render Leaves', type: 'toggle' },
            { name: 'strokeWeight', label: 'Thickness', type: 'slider', min: 0.5, max: 4, step: 0.5 },
            { group: 'Physics', icon: 'wind' },
            { name: 'swayAmount', label: 'Wind Intensity', type: 'slider', min: 0, max: 30, step: 1 },
            { name: 'swaySpeed', label: 'Wind Speed', type: 'slider', min: 0.01, max: 0.1, step: 0.01 },
            { group: 'Style', icon: 'palette' },
            {
                name: 'colorMode', label: 'Leaf Palette', type: 'palette', options: [
                    ['#22c55e', '#84cc16', '#4ade80'],
                    ['#f472b6', '#a855f7', '#ec4899'],
        ...
        draw: function (p, params) {
            ...
            const colors = this.getColors(p, params.colorMode);
            const sway = params.swayAmount > 0 ? p.sin(p.frameCount * params.swaySpeed) * params.swayAmount : 0;
        ...
        drawBranch: function (p, len, depth, params, colors, sway, currentDepth) {
            ...
            const thickness = p.map(depth, 0, params.depth, 0.5, params.strokeWeight * 4);

            p.stroke(colors[colorIndex]);
            p.strokeWeight(thickness);

            const swayOffset = sway * (params.depth - depth) * 0.15;

            p.line(0, 0, swayOffset, -len);
            p.translate(swayOffset, -len);

            const noiseVal = params.randomness > 0 ? (p.noise(currentDepth, p.frameCount * 0.01) - 0.5) * params.randomness : 0;

            p.push();
            p.rotate(params.branchAngle + swayOffset + noiseVal);
    ...
    particleSystem: {
        ...
        controls: [
            { group: 'Simulation', icon: 'sparkles' },
            { name: 'particleCount', label: 'Population', type: 'slider', min: 20, max: 400, step: 10 },
            { name: 'particleSize', label: 'Nucleus Size', type: 'slider', min: 1, max: 15, step: 1 },
            { name: 'speed', label: 'Agitation', type: 'slider', min: 0.1, max: 5, step: 0.1 },
            { group: 'Physics', icon: 'play' },
            { name: 'friction', label: 'Viscosity', type: 'slider', min: 0.8, max: 1.0, step: 0.01 },
            { name: 'gravity', label: 'Gravitation', type: 'slider', min: -0.5, max: 0.5, step: 0.05 },
            { name: 'mouseForce', label: 'Attraction', type: 'slider', min: 0, max: 4, step: 0.5 },
            { group: 'Interlinks', icon: 'link' },
            { name: 'connectionDistance', label: 'Link Range', type: 'slider', min: 20, max: 300, step: 10 },
            { name: 'showLinks', label: 'Show Strands', type: 'toggle' },
            { group: 'Aesthetics', icon: 'palette' },
            { name: 'bloom', label: 'Enable Glow', type: 'toggle' },
            {
                name: 'colorMode', label: 'Spectrum', type: 'palette', options: [
                    ['#a855f7', '#3b82f6', '#06b6d4'],
